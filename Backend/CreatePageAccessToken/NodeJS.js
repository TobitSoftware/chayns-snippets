...
app.get('/AccessToken', (req, res) => { //Get url parameters using the req-object
    try {
        let url = Server + req.query.locationId + "/AccessToken"; //build the url to request the AccesToken endpoint
        let config = {
            method: 'POST',
            headers: {
        ...
        fetch(url, config).then(response => {
            if (response.status === 200) { 
                response.json().then((data) => {
                    res.send(data.data[0].pageAccessToken); //if the request was successful, send the PageAccessToken to the client
                });
            } else {
